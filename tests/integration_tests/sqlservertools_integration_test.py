...
def test_can_create_from_query(create_test_data_datatypes, create_connection):
    ...
    connection = create_connection
    table_object = Table.from_connection(
        connection=connection, table_name="dbo.SecondTable"
    )
...
def test_columns_contain_all(
    create_test_data_datatypes, create_connection, clean_up_schema
):
    create_test_data_datatypes
    connection = create_connection
    table_object = Table.from_connection(
...
def test_can_check_if_table_exists(
...
def test_cant_create_table_in_database_if_exists(create_connection, create_test_data):
    ...
    c2 = Column(
        name="naam", type="nvarchar", nullable=True, length=255, default="onbekend"
    )
...
def test_can_insert_dict_into_table(
    create_test_data, create_connection, clean_up_schema
):
    create_test_data
    cnxn = create_connection
    ...
    data = [
        {"plaats_id": 5, "plaatsnaam": "Vlissingen", "provincie": "Zeeland"},
...
def test_insert_list_only_works_if_all_columns_are_supplied(
    create_test_data, create_connection, clean_up_schema
):
    create_test_data
    cnxn = create_connection
...
def test_list_tables_finds_tables(clean_up_schema, create_connection):
    ...
    cnxn.execute_sql(
        "drop table if exists testing.tabel_een; create table testing.tabel_een (id int)"
    )
    cnxn.execute_sql(
        "drop table if exists testing.tabel_twee; create table testing.tabel_twee (id int)"
    )
    cnxn.execute_sql(
        "drop table if exists testing.pinda_noot; create table testing.pinda_noot (id int)"
    )
    cnxn.execute_sql(
        "drop table if exists testing.noot_pinda; create table testing.noot_pinda (id int)"
    )
...
def test_list_tables_finds_tables_with_contains_match_startswith_no_match(
    create_connection,
):
    cnxn = create_connection
...
def test_list_tables_finds_tables_with_contains_no_match_startswith_match(
    create_connection,
):
    cnxn = create_connection
