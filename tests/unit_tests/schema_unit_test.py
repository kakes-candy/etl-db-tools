...
def test_column_can_print(create_column):
    ...
    assert (
        c.__str__()
        == "column: name: provincie, type: nvarchar, length: 255, precission: 4, scale: scaledef, default: onbekend"
    )
...
def test_can_add_column_method(create_column):
    c = create_column
...
def test_datetime_to_sql():
    c = Column(
        name="datumtijd", type="datetime", nullable=True, default="1900-01-01 10:00:00"
    )
...
def test_datetime2_to_sql():
    c = Column(
        name="datumtijd_twee",
        type="datetime2",
        nullable=True,
        default="1900-01-01 10:00:00",
    )
    assert c.to_sql() == "datumtijd_twee datetime2 default '1900-01-01 10:00:00'"


def test_datetime_small_to_sql():
